			...
			if(gameGift[i].act)
			{
				if((gameGift[i].y+gameGift[i].img.getHeight(null))>=Math.abs(fieldHeight-hatHeight-10) || (gameGift[i].y+gameGift[i].img.getHeight(null))>=Math.abs(fieldHeight-hatHeight2-10))
				{
					...
					else 
					{
						...
						try {
							tbreader = Files.newBufferedReader(Paths.get("localization\\locals\\TBR\\TBR.supersecretfile"));
							
						    if(recordcount>Integer.parseInt(tbreader.readLine()))
						    {
						    	FileWriter writeTBR = new FileWriter("localization\\locals\\TBR\\TBR.supersecretfile");
						    	writeTBR.write(Integer.toString(recordcount));
